...
CREATE OR REPLACE PACKAGE USS_ESR.LOAD$ASOPD_QQ
IS
    -- Author  : JSHPAK
...
/* Formatted on 8/12/2025 5:50:03 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_ESR.LOAD$ASOPD_QQ
IS
        ...
        FOR rec_ls
            IN (SELECT TRIM (SUBSTR (ls.ls_fio || '   ',
                  ...
                  FROM (SELECT DISTINCT
                               ...
                               ls.ls_nls,
                               ls.ls_spos,
                               REGEXP_REPLACE (TRIM (ls.ls_fio), '\s+', ' ')
                                   AS ls_fio,
                               ls.ls_indots,
                               ls.ls_shifr,
                               ls.ls_vid,
                ...
                BEGIN
                    ...
                    THEN
                        BEGIN
                            ...
                            -- IC #93118
                            -- для 515 КФН в документі 600 (анкета) для заявника проставляємо в nda_id =2654 (Прийомні батьки (батько/мати)= так)
                            -- для 516 КФН в документі 600 (анкета) для заявника проставляємо в nda_id =1858 (Батько/мати-вихователь дитячого будинку сімейного типу= ТАК)
                            IF NVL (rec_igd.ls_kfn, l_ls_kfn) IN
                                   ('515', '516')
                            THEN
        ...
        FOR rec_ispl
            IN (SELECT --+ index(i) index(u) index(v) use_nl(i u) use_nl(i v) index(np use_nl(i np))
                       ...
                       NVL2 (
                           np.ls_nls,
                           TO_DATE (
                               np.np_dnprav DEFAULT NULL ON CONVERSION ERROR,
                               'dd.mm.yyyy'),
                           TO_DATE (i.ispl_displ, 'dd.mm.yyyy'))
                           AS dn_start,
                       NVL2 (
                  ...
                  FROM uss_exch.v_ls_ispl_data  i
                       ...
                       LEFT JOIN uss_exch.v_ls_np_data np
                           ON     np.lfd_lfd = i.lfd_lfd
                              ...
                              AND np.np_kfn = '199'
                              AND np.Np_Pr <> '100'
                              AND TO_DATE (np.np_dnprav, 'dd.mm.yyyy') BETWEEN TO_DATE (
                                                                                   i.ispl_displ,
                                                                                   'dd.mm.yyyy')
                                                                           AND COALESCE (
                                                                                   TO_DATE (
...
--            inner join ac_detail d           on d.acd_ac = a.ac_id
    ...
    BEGIN
        ...
        --insert into tmp_work_set1(x_id1, x_id2)
        FOR c
            IN (SELECT pis.pis_id, u.ldr_lfdp
                  FROM pd_income_src pis, uss_exch.v_ls2uss u
                 WHERE     pis.pis_id = u.ldr_trg
                       AND u.ldr_code = 'USS_ESR.PD_INCOME_SRC'
                       AND pis.pis_src = p_ltype
                       AND pis.pis_pd = p_pd_id)
        LOOP
            DELETE FROM pd_income_src p
                  WHERE p.pis_id = c.pis_id;
    ...
    BEGIN
        ...
        LOOP
            BEGIN
                ...
                IF NVL (l_apr_id, 0) > 0
                THEN
                    INSERT INTO uss_esr.apr_person (aprp_apr,
                                                    aprp_tp,
                                                    aprp_inn,
                                                    history_status,
                                                    aprp_app)
                ...
                INSERT INTO uss_esr.pd_income_calc (pic_dt,
                                                    pic_pc,
                                                    pic_pd,
                                                    pic_total_income_6m,
                                                    pic_plot_income_6m,
                      ...
                      SELECT l_pd_dt,
                             ...
                             TO_NUMBER (
                                 s.perp_summax DEFAULT NULL ON CONVERSION ERROR)
            ...
            EXCEPTION
                ...
                THEN
                    l_error_msg :=
                           rec_dsd.dsd_nls
                        || '; Особа з вказаним ІПН або паспортом не знайдена;'
                        || 'Особа: '
                        || l_fam_nom
                        || ';';
