...
CREATE OR REPLACE PACKAGE USS_NDI.BLD$IMPEXP
IS
    -- Author  : VANO
...
CREATE OR REPLACE PACKAGE BODY USS_NDI.BLD$IMPEXP
IS
    ...
    FUNCTION get_pk_fields_list (p_owner VARCHAR2, p_table_name VARCHAR2)
        RETURN VARCHAR2
    IS
        l_list    VARCHAR2 (4000) := '';
        l_first   INTEGER := 1;
    BEGIN
        FOR xx
            IN (  SELECT x.column_name
                    FROM all_cons_columns x, dba_constraints y
                   WHERE     x.table_name = y.table_name
                         AND x.owner = y.owner
                         AND x.constraint_name = y.constraint_name
                         AND x.owner = UPPER (p_owner)
                         AND x.table_name = UPPER (p_table_name)
                         AND y.constraint_type = 'P'
                ORDER BY 1)
        LOOP
            l_list :=
                   l_list
    ...
    BEGIN
        ...
        l_select_sql :=
            ...
            || p_where
            || ' ORDER BY '
            || NVL (get_pk_fields_list (p_owner, p_table_name), '1');

        RETURN l_select_sql;
    END;

    FUNCTION get_field_data (p_cursor    INTEGER,
                             p_num       INTEGER,
    ...
    IS
        ...
        l_iter     INTEGER;
    BEGIN
        IF p_rec_tab (p_num).col_type = DBMS_TYPES.TYPECODE_CLOB
        THEN
            DBMS_SQL.COLUMN_VALUE (p_cursor, p_num, l_clob);
            ...
            ELSE
                ...
                FOR l_iter IN 0 .. TRUNC (DBMS_LOB.getlength (l_clob) / 1500)
                LOOP
                    l_result :=
                           l_result
                        || 'uss_ndi.tools.b64_decode('''
                        || uss_ndi.tools.b64_encode (
                               DBMS_LOB.SUBSTR (l_clob,
                                                1500,
                                                l_iter * 1500 + 1))
                        || ''')';

                    IF l_iter <> TRUNC (DBMS_LOB.getlength (l_clob) / 1500)
                    THEN
                        l_result := l_result || '||' || CHR (10);
                    END IF;
                END LOOP;
            END IF;
        ELSIF p_rec_tab (p_num).col_type <> DBMS_TYPES.TYPECODE_DATE
        THEN
            DBMS_SQL.COLUMN_VALUE (p_cursor, p_num, l_str);

            IF    l_str IS NULL
               OR UPPER (p_rec_tab (p_num).col_name) LIKE '%HS_INS'
               OR UPPER (p_rec_tab (p_num).col_name) LIKE '%HS_DEL'
               OR UPPER (p_rec_tab (p_num).col_name) LIKE '%HS_UPD'
            THEN
                l_result := 'NULL';
            ELSE
                ...
                THEN
                    IF    INSTR (l_str, CHR (10)) > 0
                       OR INSTR (l_str, CHR (13)) > 0
                    THEN
                        IF LENGTH (l_str) <= 1500
                        THEN
                            l_result :=
                                   'uss_ndi.tools.b64_decode('''
                                || uss_ndi.tools.b64_encode (l_str)
                                || ''')';
                        ELSE
                            l_result := '';

                            FOR l_iter IN 0 .. TRUNC (LENGTH (l_str) / 1500)
                            LOOP
                                l_result :=
                                       l_result
                                    || 'uss_ndi.tools.b64_decode('''
                                ...
                                IF l_iter <> TRUNC (LENGTH (l_str) / 1500)
                                THEN
                                    l_result := l_result || '||' || CHR (10);
                                END IF;
                            END LOOP;
        ...
        ELSE
            ...
            ELSE
                l_result :=
                       'to_date('''
                    || TO_CHAR (l_dt, 'DD.MM.YYYY HH24:MI:SS')
    ...
    IS
        l_select_sql        VARCHAR2 (4000);
        l_insert_template   VARCHAR2 (32000);
        l_clob              CLOB;
        ...
        l_data_dt           DATE;
        l_rows              INTEGER;
        l_field_data        VARCHAR2 (32000);
        l_data_row          VARCHAR2 (32000);
        l_f                 INTEGER;
    ...
    BEGIN
        ...
        LOOP
            ...
            CASE
                ...
                WHEN rec_tab (l_iter).col_type = DBMS_TYPES.TYPECODE_CLOB
                THEN
                    DBMS_SQL.define_column (c, l_iter, l_clob);
                ELSE
    ...
    */
    ...
    BEGIN
        ...
        WL ('sqlplus uss_ndi/uss_ndi@sonya @export_dict_and_config.sql');
    END;
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('BEGIN');
            WL (
                   '  USS_NDI.BLD$IMPEXP.make_inserts(''uss_ndi'', '''
                || tbs (i)
                || ''', '' where 1 = 1'');');
            WL ('END;');
            WL ('/');
    ...
    BEGIN
        WL (
            'prompt NVP Medirent (pavlukov) Export dictionaries for user uss_ndi@sonya');
        WL (
               'prompt Created by '
        ...
        LOOP
            ...
            WL ('DELETE FROM USS_NDI.' || LOWER (tbs (i)) || ' WHERE 1 = 1;');
            WL ('');
        END LOOP;
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user USS_NDI@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
    ...
    BEGIN
        ...
        SELECT TRUNC (COUNT (*) / 10) + 1
          INTO l_cnt
          FROM user_objects
         WHERE object_type = 'PACKAGE' AND object_name NOT LIKE '%$OP';

        l_sqlplus_line :=
            'sqlplus uss_ndi/uss_ndi@sonya @make_packages_clob.sql';
