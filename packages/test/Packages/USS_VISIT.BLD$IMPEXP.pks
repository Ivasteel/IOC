...
CREATE OR REPLACE PACKAGE USS_VISIT.BLD$IMPEXP
IS
    -- Author  : VANO
...
CREATE OR REPLACE PACKAGE BODY USS_VISIT.BLD$IMPEXP
IS
    ...
    BEGIN
        FOR xx
            IN (SELECT    'create or replace synonym &PROXY_USR..'
                 ...
                 WHERE rat_rsrc = 'RC_USS_VISIT_WEB')
        LOOP
    ...
    BEGIN
        ...
        WL ('sqlplus uss_visit/uss_visit@sonya @export_dict_and_config.sql');
    END;
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('BEGIN');
            WL (
                   '  USS_VISIT.BLD$IMPEXP.make_inserts(''uss_visit'', '''
                || tbs (i)
                || ''', '' where 1 = 1'');');
            WL ('END;');
            WL ('/');
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user USS_VISIT@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
    ...
    BEGIN
        ...
        SELECT TRUNC (COUNT (*) / 10) + 1
         ...
         WHERE     object_type = 'PACKAGE'
               AND object_name NOT IN ('DNET$COMMUNITY')
               AND object_name NOT LIKE '%$OP';

        l_sqlplus_line :=
            'sqlplus uss_visit/uss_visit@sonya @make_packages_clob.sql';
    ...
    BEGIN
        ...
        SELECT TRUNC (COUNT (*) / 40) + 1
          INTO l_cnt
          FROM uss_visit.rpt_templates;

        l_exportlob_line :=
            'ExportLOBs.exe rpt_templates rt_id rt_text dat BLOB "rt_id in (0';
        l_sqlplus_line :=
            'sqlplus uss_visit/uss_visit@sonya @make_rpt_templates_ctl.sql';
