...
CREATE OR REPLACE PACKAGE USS_ESR.API$HIST
IS
    -- Author  : VANO
    -- Created : 11.11.2021 12:12:51
    ...
    --Очищення всих тимчасових таблиць
    PROCEDURE init_work;

    -- встановлення історії
    PROCEDURE setup_history (p_mode    INTEGER,
                             p_id      t_decimal14,
                             p_start   DATE,
                             p_stop    DATE);
END API$HIST;
...
CREATE OR REPLACE PACKAGE BODY USS_ESR.API$HIST
IS
    ...
    BEGIN
        ...
        DELETE FROM tmp_real_work_list
              WHERE 1 = 1;
            ...
            SELECT x_obj,
             ...
             WHERE NOT EXISTS
                       (SELECT 1
                         ...
                         WHERE rz_obj = x_obj AND rz_begin = x_begin);
    EXCEPTION
        WHEN OTHERS
        THEN
            ROLLBACK;
