...
CREATE OR REPLACE PACKAGE USS_NDI.DNET$DIC_EXPORT_CONFIG
IS
...
END DNET$DIC_EXPORT_CONFIG;
/


GRANT EXECUTE ON USS_NDI.DNET$DIC_EXPORT_CONFIG TO DNET_PROXY
/

GRANT EXECUTE ON USS_NDI.DNET$DIC_EXPORT_CONFIG TO II01RC_USS_NDI_PORTAL
/

GRANT EXECUTE ON USS_NDI.DNET$DIC_EXPORT_CONFIG TO II01RC_USS_NDI_WEB
/

GRANT EXECUTE ON USS_NDI.DNET$DIC_EXPORT_CONFIG TO PORTAL_PROXY
/


/* Formatted on 8/12/2025 5:55:31 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_NDI.DNET$DIC_EXPORT_CONFIG
IS
    -- unused
    PROCEDURE get_net_files (p_nnf_id   IN     ndi_net_files.nnf_id%TYPE,
                             p_res         OUT SYS_REFCURSOR)
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.delete_net_files (p_nnf_id => p_nnf_id);
    END;
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.save_net_files (
            p_nnf_id           => p_nnf_id,
            p_nnf_nb           => p_nnf_nb,
            p_nnf_net          => p_nnf_net,
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.save_net_files_card (
            p_nnf_id           => p_nnf_id,
            p_nnf_nb           => p_nnf_nb,
            p_nnf_net          => p_nnf_net,
            p_nnf_name         => p_nnf_name,
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.get_export_type (p_id          => p_id,
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.query_export_type (
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.delete_export_type (p_id => p_id);
    END;
    ...
    IS
    BEGIN
        API$DIC_EXPORT_CONFIG.save_export_type (
            p_net_id           => p_net_id,
            p_net_data_tp      => p_net_data_tp,
            p_net_name         => p_net_name,
