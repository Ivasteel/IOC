...
CREATE OR REPLACE PACKAGE USS_NDI.API$DIC_EXPORT_CONFIG
IS
    Package_Name   CONSTANT VARCHAR2 (100) := 'API$DIC_EXPORT_CONFIG';
    ...
    PROCEDURE query_export_type (p_net_data_tp   IN     VARCHAR2,
                                 ...
                                 p_res              OUT SYS_REFCURSOR);
END API$DIC_EXPORT_CONFIG;
/


/* Formatted on 8/12/2025 5:55:28 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_NDI.API$DIC_EXPORT_CONFIG
IS
    -- Зберегти unused
    PROCEDURE save_net_files (
        p_nnf_id           IN     ndi_net_files.nnf_id%TYPE,
