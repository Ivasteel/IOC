...
CREATE OR REPLACE PACKAGE IKIS_SYS.BLD$IMPEXP
IS
    -- Author  : VANO
...
CREATE OR REPLACE PACKAGE BODY IKIS_SYS.BLD$IMPEXP
IS
    ...
    BEGIN
        ...
        WL ('sqlplus ikis_sys/ikis_sys@sonya @export_dict_and_config.sql');
    END;
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('BEGIN');
            WL (
                   '  IKIS_SYS.BLD$IMPEXP.make_inserts(''ikis_sys'', '''
                || tbs (i)
                || ''', '' where 1 = 1'');');
            WL ('END;');
            WL ('/');
    ...
    BEGIN
        WL (
            'prompt NVP Medirent (pavlukov) Export dictionaries for user uss_sys@sonya');
        WL (
               'prompt Created by '
        ...
        LOOP
            ...
            WL (
                'DELETE FROM IKIS_SYS.' || LOWER (tbs (i)) || ' WHERE 1 = 1;');
            WL ('');
        END LOOP;
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user IKIS_SYS@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
