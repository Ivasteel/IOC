...
CREATE OR REPLACE PACKAGE USS_DOC.BLD$IMPEXP
IS
    -- Author  : VANO
...
/* Formatted on 8/12/2025 5:47:12 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_DOC.BLD$IMPEXP
IS
    ...
    BEGIN
        ...
        WL ('sqlplus uss_doc/uss_doc@sonya @export_dict_and_config.sql');
    END;
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('BEGIN');
            WL (
                   '  USS_DOC.BLD$IMPEXP.make_inserts(''uss_doc'', '''
                || tbs (i)
                || ''', '' where 1 = 1'');');
            WL ('END;');
            WL ('/');
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('DELETE FROM USS_DOC.' || LOWER (tbs (i)) || ' WHERE 1 = 1;');
            WL ('');
        END LOOP;
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user USS_DOC@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
