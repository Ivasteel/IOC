...
CREATE OR REPLACE PACKAGE USS_ESR.BLD$IMPEXP
IS
    -- Author  : VANO
    -- Created : 10.01.2019 17:03:35
    ...
    PROCEDURE debug_table_data (p_owner        VARCHAR2,
                                p_table_name   VARCHAR2,
                                p_where        VARCHAR2,
    ...
    PROCEDURE make_av_script (p_owner VARCHAR2, p_ids_by_comma VARCHAR2);
    ...
    FUNCTION get_update_template (p_owner VARCHAR2, p_table_name VARCHAR2)
        RETURN VARCHAR2;

    PROCEDURE make_updates (p_owner        VARCHAR2,
                            p_table_name   VARCHAR2,
                            p_where        VARCHAR2);

    PROCEDURE make_package_export_cmd;

    PROCEDURE make_rpt_export_cmd;
END BLD$IMPEXP;
/


/* Formatted on 8/12/2025 5:49:17 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_ESR.BLD$IMPEXP
IS
    PROCEDURE NL
    IS
    BEGIN
    ...
    FUNCTION get_pk_field (p_owner VARCHAR2, p_table_name VARCHAR2)
        RETURN VARCHAR2
    IS
        l_rez   VARCHAR2 (100);
    BEGIN
    ...
    IS
        ...
        l_first        INTEGER := 1;
        l_pk_field     VARCHAR2 (100);
    BEGIN
        l_pk_field := get_pk_field (p_owner, p_table_name);
        l_update_sql :=
               'UPDATE '
            || LOWER (p_owner)
            || '.'
        ...
        FOR xx
            IN (  SELECT column_name
                   ...
                   WHERE     table_name = UPPER (p_table_name)
                         AND owner = UPPER (p_owner)
                         AND hidden_column = 'NO'
                         AND column_name NOT IN (UPPER (l_pk_field))
                ORDER BY column_id)
        LOOP
            l_update_sql :=
                   l_update_sql
        ...
        l_update_sql :=
               RTRIM (l_update_sql, ',' || CHR (13) || CHR (10))
            || CHR (13)
            || CHR (10)
            || '  WHERE '
            || LOWER (l_pk_field)
    ...
    BEGIN
        ...
        LOOP
            ...
            l_data_row :=
                   l_data_row
                || LPAD (SUBSTR (LOWER (rec_tab (l_iter).col_name), 1, 17),
                         17,
                         ' ')
                || '|';
        END LOOP;
        ...
        LOOP
            ...
            LOOP
                ...
                THEN
                    l_data_row :=
                           l_data_row
                        || LPAD (NVL (SUBSTR (l_field_data, 1, 17), ' '),
                                 17,
                                 ' ')
                        || '|';
                END IF;
        ...
        WL ('prompt ' || l_rows_cnt || ' records printed');
    EXCEPTION
        WHEN OTHERS
        THEN
            raise_application_error (
    ...
    BEGIN
        FOR xx
            IN (SELECT    'create or replace synonym &PROXY_USR..'
                       || z.rat_object_name
                       || ' FOR &PREFIX..'
                       || z.rat_object_name
                       || ';'    AS z_synonum_create
                  FROM ikis_sys.ikis_rsrc_attr z
                 WHERE rat_rsrc = 'RC_USS_ESR_WEB')
        LOOP
    ...
    BEGIN
        ...
        FOR xx
            IN (SELECT    rt_id
                       ...
                       || '.dat|'    AS x_row
                  FROM rpt_templates
                 WHERE rt_id IN (    SELECT REGEXP_SUBSTR (p_ids_by_comma,
                                                           '[^,]+',
                                                           1,
                                                           LEVEL)
                                       FROM DUAL
                                 CONNECT BY REGEXP_SUBSTR (p_ids_by_comma,
                                                           '[^,]+',
                                                           1,
                                                           LEVEL)
                                                IS NOT NULL))
    ...
    BEGIN
        ...
        WL ('infile *');
        WL ('replace');
        WL ('into table ' || p_owner || '.tmp$app_versions');
        WL ('fields terminated by ''|''');
        WL ('(');
        WL ('  av_id,');
        WL ('  av_tp,');
        ...
        WL ('  av_name,');
        WL ('  av_filename,');
        WL ('  av_content lobfile(av_filename) terminated by eof');
        WL (')');
        WL ('begindata');
        ...
        FOR xx
            IN (SELECT    av_id
                       ...
                       || av_id
                       || '.dat|'    AS x_row
                  FROM ikis_sys.ikis_app_docs
                 WHERE     av_ss_code = 'USS_ESR'
                       AND av_id IN
                               (    SELECT REGEXP_SUBSTR (p_ids_by_comma,
                                                          '[^,]+',
                                                          1,
                                                          LEVEL)
                                      FROM DUAL
    ...
    BEGIN
        ...
        WL ('sqlplus uss_esr/uss_esr@sonya @export_dict_and_config.sql');
    END;
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('BEGIN');
            WL (
                   '  USS_ESR.BLD$IMPEXP.make_inserts(''uss_esr'', '''
                || tbs (i)
                || ''', '' where 1 = 1'');');
            WL ('END;');
            WL ('/');
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('DELETE FROM USS_ESR.' || LOWER (tbs (i)) || ' WHERE 1 = 1;');
            WL ('');
        END LOOP;
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user USS_ESR@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
    ...
    IS
        l_select_sql        VARCHAR2 (4000);
        l_update_template   VARCHAR2 (4000);
    ...
    BEGIN
        DBMS_OUTPUT.enable (NULL);
        l_update_template := get_update_template (p_owner, p_table_name);
        l_select_sql := get_select_template (p_owner, p_table_name, p_where);
        ...
        WL ('prompt ' || l_rows_cnt || ' records updated');
    EXCEPTION
        WHEN OTHERS
        THEN
            raise_application_error (
    ...
    BEGIN
        ...
        SELECT TRUNC (COUNT (*) / 10) + 1
         ...
         WHERE     object_type = 'PACKAGE'
               AND object_name NOT IN ('API$ACC_TEST',
                                       'SLF$TEST',
                                       'API$CALC_RIGHT_AT',
                                       'CALC$PAYROLL')
        ...
        l_sqlplus_line :=
            'sqlplus uss_esr/uss_esr@sonya @make_packages_clob.sql';
    ...
    IS
        l_cnt              INTEGER;
        l_exportlob_line   VARCHAR2 (250);
        l_sqlplus_line     VARCHAR2 (250);
    BEGIN
        WL (
            'REM Кожна змінна listN не може бути більша за 200 символів!!! І повинна містити принаймні 0');
        WL (CHR (10));

        SELECT TRUNC (COUNT (*) / 40) + 1
          INTO l_cnt
          FROM uss_esr.rpt_templates;

        l_exportlob_line :=
            'ExportLOBs.exe rpt_templates rt_id rt_text dat BLOB "rt_id in (0';
        l_sqlplus_line :=
            'sqlplus uss_esr/uss_esr@sonya @make_rpt_templates_ctl.sql';
        ...
        FOR xx
            IN (  SELECT DISTINCT
                         ...
                         || '='
                         || LISTAGG (LOWER (rt_id), ',')
                                WITHIN GROUP (ORDER BY line, rt_id)
                                OVER (PARTITION BY line)    AS y_line
                    FROM (    SELECT LEVEL     AS line
                                FROM DUAL
                          CONNECT BY LEVEL <= l_cnt),
                         (  SELECT rt_id, TRUNC (ROWNUM / (40 + 1)) + 1 AS x_line
                              FROM uss_esr.rpt_templates
                          ORDER BY rt_id)
                   WHERE line = x_line(+)
                ORDER BY 1)
        LOOP
            l_exportlob_line :=
                l_exportlob_line || ', %list' || xx.line || '%';
            l_sqlplus_line :=
                l_sqlplus_line || ' ''%list' || xx.line || '%''';
            WL (xx.y_line);
        END LOOP;

        l_exportlob_line := l_exportlob_line || ')"';
