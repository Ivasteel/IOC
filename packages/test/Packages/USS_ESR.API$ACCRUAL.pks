...
CREATE OR REPLACE PACKAGE USS_ESR.API$ACCRUAL
IS
    -- Author  : VANO
...
GRANT EXECUTE ON USS_ESR.API$ACCRUAL TO II01RC_USS_ESR_INTERNAL
/

GRANT EXECUTE ON USS_ESR.API$ACCRUAL TO IKIS_RBM
/

GRANT EXECUTE ON USS_ESR.API$ACCRUAL TO OPERVIEIEV
/

GRANT EXECUTE ON USS_ESR.API$ACCRUAL TO USS_PERSON
/

GRANT EXECUTE ON USS_ESR.API$ACCRUAL TO USS_RPT
/

GRANT EXECUTE ON USS_ESR.API$ACCRUAL TO USS_VISIT
/


/* Formatted on 8/12/2025 5:48:35 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_ESR.API$ACCRUAL
IS
    ...
    BEGIN
        ...
        ---Підготовлюється правильна табличку tmp_pd_accrual_period, тобто формується історія дії рішень, моделюється перевід рішення p_pd_id в стан Призначено.
        calc_accrual (1,
                      3,
                      l_pc_id,
                      NULL,
    ...
    BEGIN
        ...
        DELETE FROM tmp_ac_dn_nl
              WHERE 1 = 1;

        DELETE FROM tmp_wac_calc_params
              WHERE 1 = 1;
    END;
        ...
        ELSE
            INSERT INTO tmp_ac_nst_list (x_nst)
                SELECT DISTINCT ncc_nst
                 ...
                 WHERE     ncc_nst NOT IN (1101) --Не нараховуємо по послугам "дотяжка ВПП ООН до 3250".
                       AND (ncc_nst NOT IN (664, 20) OR p_calc_mode IN (1, 3)); --Для немасового розрахунку - всі послуги
        END IF;
    END;
    ...
    BEGIN
        ...
        THEN
            SELECT /*+index(pd_payment IFK_PDD_PDP)*/
                   SUM (pdd_value)
              INTO l_exclude_sum
              FROM pd_detail, uss_ndi.v_ndi_nst_dn_exclude
            ...
            THEN
                RETURN (l_pdp.pdp_sum - l_exclude_sum) / l_pdp.pdp_sum;
            ELSE
                RETURN 1;
            END IF;
        ELSE
    ...
    BEGIN
        ...
        THEN                             --| dn_first_step--start_usable_sum |
            diag_msg (
                'ndn|  month   | full_income | dn_second_step | sd_usable_sum | npt | used_sum | id | npt_usable_sum | nst |      pd | used_by_dn | pd_income |  sd_pa |');
            ...
            LOOP
                diag_msg (
                       p (x.sd_ndn, 3)
                    || p (TO_CHAR (x.sd_month, 'DD.MM.YYYY'), 10)
    ...
    BEGIN
                             ...
                             NVL (
                                 MAX (
                                     CASE
                                         ...
                                         THEN
                                             API$ACCRUAL.get_exclude_sum_by_pd (
                                                 a_pd,
                                                 a_npt,
                                                 nnnc_id,
                                                 a_start_dt)
        ...
        LOOP
            ...
            SELECT xpa_bp_start_dt
              INTO l_ac_month
              FROM tmp_ac_pa
             WHERE xpa_id = l_deduction.s_pa;
            ...
            l_income_found_ori := l_income_found;
                ...
                THEN  --Для переплат - ліміт на основі параметрів відрахування
                    CASE l_dn_params.d_tp
                        ...
                        THEN
                            ...
                            l_pd_limit := l_dn_params.d_value;
                        WHEN 'SD'
                        THEN
                            l_limit :=
                                  l_income.sd_full_income
                    ...
                    diag_msg (
                           'S_DN='
                        || l_deduction.S_DN
                        || ';Сума ліміту по боргам: l_debt_limit='
                        || l_debt_limit);

                    /*IF l_deduction.s_op = 40 THEN --Це старі борги s_op=40
                      l_limit := l_debt_limit;
                      diag_msg('Ліміт, зкоригований з урахуванням того. що це борг: l_limit='||l_limit);
                    END IF;*/
                    ...
                    THEN --Якщо обрахований абсолютний ліміт відрахування менший за використану раніше суму - то дохід не підходить, все вже використано
                        ...
                        l_income_found := 0;
                        diag_msg (
                            'Якщо обрахований абсолютний ліміт відрахування менший за використану раніше суму - то дохід не підходить, все вже використано');
                    ...
                    diag_msg (
                           'Ліміт, зкоригований з урахуванням l_income.sd_used_sum: l_limit='
                        || l_limit);
            ...
            THEN
                ...
                INSERT INTO tmp_ac_detail (a_pc,
                         ...
                         VALUES (
                                    ...
                                    '6',
                                    'T',
                                    l_deduction.s_ori_start_dt,
                                    l_deduction.s_ori_stop_dt);
                ...
                UPDATE tmp_ac_income_for_dn
                 ...
                 WHERE     sd_month = l_income.sd_month
                       ...
                       AND sd_npt = l_income.sd_npt; --Для переплотного типу AS/PL - беруться з загальної суми нарахуваннь, а не окремо по кожному NPT

                /*AND ((l_dn_params.d_dn_tp = 'D' AND sd_npt = l_income.sd_npt)
                  OR (l_dn_params.d_dn_tp IN ('R', 'HM')
        ...
        INSERT INTO tmp_ac_detail (a_pc,
            ...
            SELECT s_pc,
                   s_pa,
                   CASE
                       WHEN s_op = 40 AND s_sum_tp = 'DEBT' THEN 5
                       ELSE 9
                   END,
                   s_npt,
                   xpa_bp_start_dt,
                   ...
                   s_ori_start_dt,
                   s_ori_stop_dt
              FROM tmp_ac_dn_src z, tmp_ac_pa
             WHERE s_st IN ('W', 'N') AND s_pa = xpa_id AND s_sum > 0;
    END;
    ...
    BEGIN
        ...
        INSERT INTO tmp_ac_dn_params (d_dn,
            ...
            SELECT dn_id,
             ...
             WHERE     x_id = dn_pc
                   ...
                   AND (   (dn_tp IN ('R', 'HM') AND x_start_dt = nm_start_dt)
                        OR (    dn_tp IN ('D')
                            AND NVL (dnd_stop_dt, x_stop_dt) >= x_start_dt
                            AND dnd_start_dt <= x_stop_dt
                            AND NVL (dnd_stop_dt, x_stop_dt) >= nm_start_dt
                            AND dnd_start_dt <= nm_stop_dt)
                        OR (    dn_tp IN ('D')
                            AND x_start_dt = nm_start_dt
                            AND NVL (dnd_stop_dt, x_stop_dt) < x_start_dt))
                   ...
                   AND dn_pa = xpa_id
                   AND xpa_id = pa_id
                   AND NVL (dn_params_src, 'DND') = 'DND' --NOT EXISTS (SELECT 1 FROM dn_person dnp WHERE dnp_dn = dn_id AND dnp.history_status = 'A'/* AND dnp_value > 0*/)
        ...
        INSERT INTO tmp_ac_detail (a_pc,
            ...
            SELECT DISTINCT d_pc,
                            ...
                            xpa_bp_stop_dt,
                            '2'
              FROM tmp_wac_ids, tmp_ac_dn_params ma, tmp_ac_pa
             WHERE     x_id = d_pc
                   AND xpa_id = d_pa
                   AND d_start_debt > 0
        ...
        diag_msg (
            'Знаходимо суми розрахункових доходів на періоди. Повні та новонараховані');
        ...
        INSERT INTO tmp_ac_income (tai_pc,
              ...
              SELECT a_pc,
                     ...
                     0,
                     SUM (CASE
                              WHEN a_need_write_2_base = 'T' AND a_op IN (1, 2)
                              THEN
                                    a_sum
            ...
            SELECT d_pc, d_pa, DECODE (x_row, 1, d_start_dt, d_stop_dt + 1)
              FROM tmp_ac_dn_params,
                   (    SELECT LEVEL     AS x_row
                          FROM DUAL
                    CONNECT BY LEVEL < 3)
             WHERE d_start_dt IS NOT NULL
            UNION ALL
            SELECT /*+ full (wac) use_nl(wac pd) use_nl(pd acd) */
                   wac.x_id, pd_pa, acd_start_dt           --, acd_stop_dt + 1
              FROM uss_esr.tmp_wac_ids    wac,
                   uss_esr.accrual        ac,
                   uss_esr.ac_detail      acd,
                   uss_esr.pc_decision    pd,
                   uss_esr.tmp_work_ids4  npt
             WHERE     acd_ac = ac_id
                   AND ac_pc = wac.x_id
                   ...
                   AND (   g_pa_mode_calc = 0
                        OR (    g_pa_mode_calc = 1
                            AND EXISTS
                                    (SELECT 1
                                       FROM uss_esr.tmp_work_pa_ids
                                      WHERE pd_pa = x_pa)));

        diag_msg ('Знаходимо унікальний набір дат розривів');
        ...
        DELETE FROM tmp_dn_dates
          ...
          --523 операція - розрахункова повна сума відрахування (для подальшого порівняння з сумами в проводках і генерації "дельт")
          INSERT ALL
            WHEN q_sum > 0
            THEN
            ...
            SELECT q_pc,
                   ...
                   q_ac_stop_dt,
                   '3'                                  AS q_stage,
                   'T'                                  AS q_need_write_2_base,
                   q_nst,
                   q_corrected_by_limit_base_dn_sum     AS q_base_sum,
                   q_base_month_sum
              FROM (SELECT x_id
                               AS q_pc,
                           q_pa,
                           ...
                           q_start_dt,
                           q_stop_dt,
                           tai_sum * d_can_use_koef
                               AS q_month_sum,
                           tai_base_sum * d_can_use_koef
                               AS q_base_month_sum,
                           NULL
                               AS q_delta_pay,
                           q_dn,
                           ...
                           ROUND (
                               ...
                               2)
                               AS q_corrected_by_limit_dn_sum,
                           ROUND (
                                 q_base_dn_sum
                               * q_part_of_income
                               * d_can_use_koef,
                               2)
                               AS q_corrected_by_limit_base_dn_sum
                      FROM tmp_wac_ids,
                           uss_ndi.v_ndi_months,
                           (SELECT tai_pc
                                       AS q_pc,
                             ...
                             WHERE     tai_pc = d_pc
                                   ...
                                   AND td_begin BETWEEN tai_start_dt
                                                    AND tai_stop_dt
                                   --AND d_value > 0
                                   AND d_calc_step = 'F'/*AND tai_sum > 0*/
                                                        )
                     WHERE     x_id = q_pc
                           AND nm_start_dt BETWEEN x_start_dt AND x_stop_dt
        ...
        diag_msg ('Знаходимо суми зафіксованих відрахувань на періоди');
        ...
        INSERT INTO tmp_ac_detail (a_pc,
              ...
              SELECT td_pc,
               ...
               WHERE     d.history_status = 'A'
                     ...
                     AND acd_pd = pd_id
                     AND td_pa = pd_pa
                     AND acd_start_dt <= td_end
                     AND acd_stop_dt >= td_begin
                     AND acd_op IN (SELECT op_id
                                      FROM uss_ndi.v_ndi_op
                                     WHERE op_tp1 IN ('DN'))
                     AND EXISTS
            ...
            UNION ALL
              SELECT td_pc,
                     ...
                     SUM (CASE
                              WHEN 1 = 1
                              THEN
                                    uss_esr.API$ACCRUAL.xsign (a_op)
                                  * -1
                                  * a_sum
                     ...
                     'F',
                     a_nst
                FROM uss_esr.tmp_ac_detail, uss_esr.tmp_dn_dates
               WHERE     a_pc = td_pc
                     AND td_pa = a_pa
                     AND a_start_dt <= td_end
                     AND a_stop_dt >= td_begin
                     AND a_op IN (SELECT op_id
                                    FROM uss_ndi.v_ndi_op
                                   WHERE op_tp1 IN ('DN'))
            GROUP BY td_pc,
                     td_pa,
        ...
        diag_msg ('Знаходимо суми виплачених відрахувань на періоди');
        ...
        INSERT INTO tmp_ac_detail (a_pc,
              ...
              SELECT td_pc,
               ...
               WHERE     d.history_status = 'A'
                     ...
                     AND acd_op IN (SELECT op_id
                                      FROM uss_ndi.v_ndi_op
                                     WHERE op_tp1 IN ('DN') OR op_id IN (33))
                     AND EXISTS
                             (SELECT 1
                                FROM tmp_ac_detail
                               WHERE     td_pc = a_pc
        ...
        diag_msg (
            'Знаходимо суми зафіксованих але не виплачених коригуваннь відрахувань на періоди');
        ...
        diag_msg (
            'Знаходимо суми зафіксованих доручень повреннень надміру стягнених відрахувань на періоди');
        ...
        diag_msg (
            'Розраховуємо суми доплат та повереннь відпрахувань на періоди');

          INSERT ALL
            WHEN ABS (k_sum_dpp_corr_plus) > 0.01
            THEN               --доплата закладу відрахування на держутримання
                INTO tmp_ac_detail (a_pc,
                                    a_pa,
                                    a_op,
                                    a_npt,
                  ...
                  VALUES (k_pc,
                          ...
                          'T',
                          k_nst)
            WHEN ABS (k_sum_dpp_corr_minus) > 0.01
            THEN --коригування нарахованого закладу відрахування на держутримання
                INTO tmp_ac_detail (a_pc,
                                    a_pa,
                                    a_op,
                                    a_npt,
                  ...
                  VALUES (k_pc,
                          ...
                          k_start_dt,
                          k_stop_dt,
                          k_sum_dpp_corr_minus,
                          k_dn,
                          k_pd,
                          '3.6',
                          'T',
                          k_nst)
            WHEN ABS (k_sum_dpp_corr_return) > 0.01
            THEN   --повернення закладом частини відрахування на держутримання
                INTO tmp_ac_detail (a_pc,
                                    a_pa,
                                    a_op,
                                    a_npt,
                  ...
                  VALUES (k_pc,
                          ...
                          k_start_dt,
                          k_stop_dt,
                          k_sum_dpp_corr_return,
                          k_dn,
                          k_pd,
                          '3.6',
                          'T',
                          k_nst)
            WHEN ABS (k_sum_pc_corr_return) > 0.01
            THEN --повернення особі надміру стягнених коштів у вигляді відрахування на держутримання
                INTO tmp_ac_detail (a_pc,
                                    a_pa,
                                    a_op,
                                    a_npt,
            ...
            SELECT k_pc,
                   ...
                   k_nst,
                   CASE
                       WHEN     k_new_calc_sum > k_old_payed_sum
                            AND k_old_payed_sum >= k_old_saved_sum
                       THEN
                           k_new_calc_sum - k_old_payed_sum
                       WHEN k_new_calc_sum > k_old_saved_sum
                       THEN
                           k_new_calc_sum - k_old_saved_sum
                       ELSE
                           0
                   END    AS k_sum_dpp_corr_plus,
                   CASE
                       WHEN     k_old_payed_sum <= k_new_calc_sum
                            AND k_new_calc_sum < k_old_saved_sum
                       THEN
                           k_old_saved_sum - k_new_calc_sum
                       ELSE
                           0
                   END    AS k_sum_dpp_corr_minus,
                   CASE
                       WHEN     k_new_calc_sum <= k_old_payed_sum
                            AND k_old_payed_sum <= k_old_saved_sum
                       THEN
                           k_old_saved_sum - k_new_calc_sum
                       WHEN     k_new_calc_sum <= k_old_saved_sum
                            AND k_old_saved_sum <= k_old_payed_sum
                       THEN
                           k_old_saved_sum - k_new_calc_sum
                       WHEN     k_new_calc_sum > k_old_payed_sum
                            AND k_old_payed_sum > k_old_saved_sum
                       THEN
                           k_old_saved_sum - k_old_payed_sum
                       ELSE
                           0
                   END    AS k_sum_dpp_corr_return,
                   CASE
                       WHEN     k_new_calc_sum <= k_old_payed_sum
                            AND k_old_payed_sum <= k_old_saved_sum
                       THEN
                           k_old_payed_sum - k_new_calc_sum
                       WHEN     k_new_calc_sum <= k_old_saved_sum
                            AND k_old_saved_sum <= k_old_payed_sum
                       THEN
                           k_old_payed_sum - k_new_calc_sum
                       ELSE
                           0
                   END    AS k_sum_pc_corr_return
              FROM (  SELECT a_pc          AS k_pc,
                             a_pa          AS k_pa,
                             a_npt         AS k_npt,
                             a_start_dt    AS k_start_dt,
                             a_stop_dt     AS k_Stop_dt,
                             a_dn          AS k_dn,
                             MAX (a_pd)    AS k_pd,
                             a_nst         AS k_nst,
                             NVL (
                                 SUM (
                                     CASE WHEN a_op = 501 THEN a_sum ELSE 0 END),
                                 0)        AS k_new_calc_sum,
                             NVL (
                                 SUM (
                                     CASE WHEN a_op = 502 THEN a_sum ELSE 0 END),
                                 0)        AS k_old_saved_sum,
                             NVL (
                                 SUM (
                                     CASE WHEN a_op = 503 THEN a_sum ELSE 0 END),
                                 0)        AS k_old_payed_sum,
                             NVL (
                                 SUM (
                                     CASE WHEN a_op = 504 THEN a_sum ELSE 0 END),
                                 0)        AS k_fixed_corr_sum,
                             NVL (
                                 SUM (
                                     CASE WHEN a_op = 505 THEN a_sum ELSE 0 END),
                                 0)        AS k_fixed_return_sum
                        FROM tmp_ac_detail
                       WHERE a_op IN (501,
                                      502,
                                      503,
                    ...
                    GROUP BY a_pc,
                             ...
                             a_dn,
                             a_nst)
             WHERE k_new_calc_sum <> k_old_saved_sum;

        UPDATE tmp_ac_detail
           SET (a_ac_start_dt, a_ac_stop_dt) =
                   (SELECT xpa_bp_start_dt, xpa_bp_stop_dt
                      FROM tmp_ac_pa
                     WHERE xpa_id = a_pa)
         WHERE a_op IN (30,
                        31,
        ...
        INSERT INTO tmp_ac_dn_src (s_pc,
            ...
            SELECT x_id
                   ...
                   q_start_dt,
                   q_stop_dt,
                   ROUND (q_month_sum * q_part_of_month * d_can_use_koef, 2)
                       AS q_dn_sum,
                   q_dn,
                   q_pd,
                   '2',
        ...
        INSERT INTO tmp_ac_dn_src (s_pc,
            ...
            SELECT x_id,
                   ...
                   q_start_dt,
                   q_stop_dt,
                   ROUND (q_sum * d_part_of_month, 2)     AS a_sum,
                   q_dn,
                   q_pd,
                   '3',
                   d_ndn_order,
              ...
              FROM tmp_wac_ids,
                   ...
                   (SELECT x_id          AS q_pc,
                           ...
                           d_dn          AS q_dn,
                           NULL          AS q_npt,
                           d_start_dt    AS q_start_dt,
                           d_stop_dt     AS q_stop_dt,
                           CASE d_tp
                               WHEN 'AS'
                               THEN
                                   d_value
                     ...
                     WHERE     x_id = d_pc
                           ...
                           AND NOT EXISTS
                                   (SELECT 1
                                      FROM uss_esr.dn_month_usage
                                     WHERE     dnu_dn = d_dn
                                           AND dnu_stop_dt >= d_start_dt
                                           AND dnu_start_dt <= d_stop_dt
                                           AND dnu_sum > 0))
             WHERE     x_id = q_pc
                   AND q_start_dt BETWEEN nm_start_dt AND nm_stop_dt
                   AND nm_start_dt BETWEEN x_start_dt AND x_stop_dt;

        --Розраховуємо суму "не менше ніж" для тих відрахуваннь, в яких вона вказана.
        INSERT INTO tmp_ac_dn_nl (n_pc,
                                  n_pa,
                                  n_start_dt,
                                  n_stop_dt,
                                  n_sum,
            ...
            SELECT x_id,
                   ...
                   q_start_dt,
                   q_stop_dt,
                   ROUND (NVL (q_nl_sum, 0) * d_part_of_month, 2)
                       AS a_sum,
                   q_dn,
                   q_pd,
                   '4',
              ...
              FROM tmp_wac_ids,
                   ...
                   (SELECT x_id          AS q_pc,
                     ...
                     WHERE     x_id = d_pc
                           ...
                           AND x_start_dt <= d_stop_dt
                           AND x_stop_dt >= d_start_dt
                           AND d_nl_value > 0 --"Не менше ніж" розраховується тільки якщо вказано параметри
                           AND d_nl_tp IS NOT NULL
                           AND d_calc_step = 'S'
                           AND d_dn_tp = 'D'
                           AND NOT EXISTS
                                   (SELECT 1
        ...
        --Перевіряємо, чи не перевищує сума "не менш ніж" суму відрахуваннь по всім кодам виплат по основним параметрам.
        --Якщо перевищує, різницю вставляємо як додаткову суму боргу по типу 40.
        --Ключ: період + ід оригінального відрахування, а параметри відрахування з таблиці tmp_ac_dn_params.
        INSERT INTO tmp_ac_dn_src (s_pc,
                                   s_pa,
                                   s_op,
                                   s_npt,
            ...
            WITH
                ...
                AS
                    (  SELECT s_d_id,
                         ...
                         FROM tmp_ac_dn_src
                        WHERE s_op = 123
                     GROUP BY s_d_id, s_start_dt),
                calc_by_nl
                AS
                    (  SELECT n_d_id, n_start_dt, SUM (n_sum) AS n_nl_sum
                         FROM tmp_ac_dn_nl
                     GROUP BY n_d_id, n_start_dt)
            SELECT d_pc,
                   d_pa,
                   123,
                   NULL,
                   ...
                   d_nst,
                   'CURRM'
              FROM calc_by_dn, calc_by_nl, tmp_ac_dn_params
             WHERE     s_d_id = n_d_id
                   AND s_start_dt = n_start_dt
                   AND n_nl_sum > s_sum
                   AND n_d_id = d_id;

        --Оновлюємо збережені записи використання на суму відрахування, яку розраховано як новий поточний борг
        --!!! для абсолютних сумм треба розраховувати різницю, але це наступним етапом
        UPDATE uss_esr.dn_month_usage
           SET dnu_sum =
                     dnu_sum
                   + (SELECT SUM (  s_sum
                                  * TOOLS.p_o_p2 (s_start_dt,
                                                  s_stop_dt,
                                                  dnu_start_dt,
                                                  dnu_stop_dt))
                        FROM tmp_ac_dn_src
                       WHERE     dnu_dn = s_dn
                             AND dnu_stop_dt >= s_start_dt
                             AND dnu_start_dt <= s_stop_dt
                             AND s_sum_tp = 'CURRM'
                             AND s_stage IN ('3', '4'))
         WHERE EXISTS
                   (SELECT 1
        ...
        --Записуємо в таблицю використання нові відрахування
        INSERT INTO uss_esr.dn_month_usage (dnu_id,
                                            dnu_dn,
                                            dnu_start_dt,
                                            dnu_stop_dt,
                                            dnu_sum)
                     ...
                     AND NOT EXISTS
                             (SELECT 1
                               ...
                               WHERE     dnu_dn = s_dn
                                     AND dnu_stop_dt >= s_start_dt
                                     AND dnu_start_dt <= s_stop_dt)
            GROUP BY s_dn, s_start_dt, s_stop_dt;
    END;
    ...
    BEGIN
                ...
                SELECT DISTINCT pc_id
                 ...
                 WHERE     pc_id = p_pc_id
                       ...
                       AND (   (g_pa_mode_calc = 0 AND pc.com_org = x_id)
                            OR (g_pa_mode_calc = 1 AND pa_org = x_id));
        ...
        ELSE
            ...
            THEN
                SELECT COUNT (DISTINCT pc_id)
                  INTO l_cnt
                  FROM tmp_work_ids tpc, personalcase pc, tmp_com_orgs orgs
                 WHERE tpc.x_id = pc_id AND pc.com_org = orgs.x_id;
            ELSIF g_pa_mode_calc = 1
            THEN
                SELECT COUNT (DISTINCT pa_pc)
                  INTO l_cnt
                  FROM tmp_work_ids tpc, tmp_com_orgs orgs, pc_account
                 WHERE tpc.x_id = pa_pc AND pa_org = orgs.x_id;
            END IF;
        END IF;
        ...
        THEN
            SELECT COUNT (*)
             ...
             WHERE EXISTS
                       (SELECT 1
                          FROM tmp_com_orgs, pc_decision, pc_account
                         WHERE     pd_pa = x_pa
                               AND pd_pa = pa_id
                               AND pa_org = x_id);
        ...
        calc_write_message (
            p_calc_mode,
            'Обраховую ОР та період нарахувань для кожного ОР в розрахунку.',
            'I',
            g_messages);

        DELETE FROM tmp_ac_pa
              WHERE 1 = 1;

        IF g_pa_mode_calc = 1
        THEN
            INSERT INTO tmp_ac_pa (xpa_id, xpa_pc, xpa_bp_start_dt)
                SELECT x_pa,
                       pa_pc,
                       (SELECT MAX (bp_month)
                          FROM billing_period
        ...
        SELECT COUNT (*) INTO l_cnt FROM tmp_ac_pa;

        IF l_cnt = 0
        THEN
            raise_application_error (-20000,
                                     'Не знайдено ОР для розрахунку!');
        END IF;

        UPDATE tmp_ac_pa
           SET xpa_bp_stop_dt = LAST_DAY (xpa_bp_start_dt)
         WHERE 1 = 1;
        ...
        INSERT INTO tmp_wac_calc_params (wcp_pc)
            SELECT x_id FROM tmp_work_ids;

        UPDATE tmp_wac_calc_params z
           SET z.wcp_paq_start_dt =
                   (SELECT MIN (q_start_dt)
                      FROM tmp_pc_accrual_queue
                     WHERE q_pc = wcp_pc AND q_st = 'W')
         WHERE 1 = 1;

        UPDATE tmp_wac_calc_params z
           SET z.wcp_bp_start_dt =
                   (SELECT MIN (xpa_bp_start_dt)
                      FROM tmp_ac_pa
                     WHERE xpa_pc = wcp_pc)
         WHERE 1 = 1;

        UPDATE tmp_wac_calc_params z
           SET z.wcp_bp_stop_dt =
                   (SELECT MAX (xpa_bp_stop_dt)
                      FROM tmp_ac_pa
                     WHERE xpa_pc = wcp_pc)
         WHERE 1 = 1;
        ...
        UPDATE tmp_wac_calc_params z
           SET z.wcp_ac_count =
                   (SELECT COUNT (*)
                      FROM accrual
                     WHERE     ac_pc = wcp_pc
                           AND ac_month = wcp_bp_start_dt
                           AND ac_st IN
                                   ('W',
                                    'R',
                                    DECODE (g_bp_class, 'V', 'RV', 'RP')))
         WHERE 1 = 1;
        ...
        INSERT INTO tmp_wac_ids (x_id,
            ...
            SELECT wcp_pc,
                   p_calc_mode,
                   GREATEST (
                       TRUNC (NVL (wcp_paq_start_dt, wcp_bp_start_dt), 'MM'),
                       DECODE (g_bp_class,
                               'V', TO_DATE ('01.01.2022', 'DD.MM.YYYY'),
                               TO_DATE ('01.03.2022', 'DD.MM.YYYY'))),
                   wcp_bp_stop_dt
              FROM tmp_wac_calc_params
        ...
        /*INSERT INTO tmp_wac_ids (x_id, x_calc_mode, x_start_dt,
                                 x_stop_dt\*, x_ac_start_dt, x_ac_stop_dt*\)
          SELECT x_id, p_calc_mode, GREATEST(TRUNC(NVL((SELECT MIN(q_start_dt)
                                                        FROM tmp_pc_accrual_queue
                                                        WHERE q_pc = x_id
                                                          AND q_st = 'W'), (SELECT MIN(xpa_bp_start_dt) FROM tmp_ac_pa WHERE xpa_pc = x_id)), 'MM'),
                                             DECODE(g_bp_class, 'V', to_date('01.01.2022', 'DD.MM.YYYY'), to_date('01.03.2022', 'DD.MM.YYYY'))),
                 (SELECT MAX(xpa_bp_stop_dt) FROM tmp_ac_pa WHERE xpa_pc = x_id)\*, v_ac_month, LAST_DAY(v_ac_month)*\
          FROM tmp_work_ids
          WHERE (p_calc_mode = 3 --для попереднього розрахунку - можна будь-яку справу брати
              OR (p_calc_mode IN (1, 2) --для індивідуального/масового - беруться тільки ті справи, по яких немає нарахувань в станах "передано на візування" "завізовано".
                AND NOT EXISTS (SELECT 1
                                FROM accrual
                                WHERE ac_pc = x_id
                                  AND ac_month = (SELECT MIN(xpa_bp_start_dt) FROM tmp_ac_pa WHERE xpa_pc = x_id)
                                  AND ac_st IN ('W', 'R', DECODE(g_bp_class, 'V', 'RV', 'RP')))));*/
        ...
        --Видаляємо зайві записи, аби tmp_wac_calc_params можна було використовувати в різних запитах під час розрахунку
        DELETE FROM tmp_wac_calc_params
              WHERE NOT EXISTS
                        (SELECT 1
                           FROM tmp_wac_ids
        ...
        THEN
            SELECT COUNT (*), MAX (ac_id)
              INTO l_cnt, l_id
              FROM accrual, tmp_wac_ids, tmp_ac_pa
             WHERE     ac_pc = x_id
                   AND ac_st NOT IN
                           ('E', DECODE (g_bp_class, 'V', 'RP', 'RV'))
                   AND ac_month = xpa_bp_start_dt
                   AND xpa_pc = x_id;

            IF l_cnt > 0
            THEN
                SELECT    'Розраховувати справу ('
                       || pc_num
                       || '), по якій є нарахування не в стані "Редагується" на '
                       || TO_CHAR (ac_month, 'MM.YYYY')
                       || ' - не можна! (Всього таких передано в розрахунок '
                       || l_cnt
                       || ')...'
                       || l_id
                  INTO l_msg
                  FROM accrual, personalcase
                 WHERE ac_id = l_id AND ac_pc = pc_id;
        ...
        THEN
            ...
            INSERT INTO tmp_ac_to_del (x_acd,
                                       x_ac,
                                       x_dn,
                                       x_ed)
                SELECT acd_id,
                       acd_ac,
                       CASE
                           WHEN acd_op IN (6, 30) THEN acd_dn_src
                           ELSE acd_dn
                       END,
                       acd_ed
                  FROM ac_detail
                 WHERE     acd_op IN (1,
                       ...
                       AND EXISTS
                               (SELECT 1
                                  FROM accrual, tmp_wac_calc_params
                                 WHERE     acd_ac = ac_id
                                       AND ac_pc = wcp_pc
                                       AND ac_st IN
                                               ('E',
                       ...
                       AND (   EXISTS
                                   (SELECT 1
                                      FROM pc_decision, tmp_ac_pa
                                     WHERE pd_pa = xpa_id AND acd_pd = pd_id)
                            OR EXISTS
                                   (SELECT 1
                                      FROM deduction, tmp_ac_pa
                                     WHERE dn_pa = xpa_id AND acd_dn = dn_id)
                            OR EXISTS
                                   (SELECT 1
                                      FROM errand, tmp_ac_pa
                                     WHERE ed_pc = xpa_pc AND acd_ed = ed_id));
            ...
            INSERT INTO tmp_work_set3 (x_id1, x_sum1)
                SELECT dn_id,
                         dn_debt_current
                       + NVL (
                             (SELECT SUM (acd_sum)
            ...
            /*--Відновлюємо суму поточного боргу відрахувань з рядків операцій 123/124, які видаляємо
            UPDATE deduction
              SET dn_debt_current = dn_debt_current + NVL((SELECT SUM(acd_sum)
                                                           FROM ac_detail, tmp_ac_to_del
                                                           WHERE acd_dn = dn_id
                                                             AND acd_id = x_acd
                                                             AND acd_op IN (124, CASE WHEN dn_tp = 'D' THEN 124 ELSE 123 END)), 0)
              WHERE EXISTS (SELECT 1 FROM tmp_ac_to_del WHERE x_dn = dn_id)
                AND dn_tp IN ('R', 'HM');*/
            ...
            /*--Оновлюємо поле "поточний борг" у відрахуванні на основі нерознесених залишків по ід-у операції 5
            UPDATE deduction
              SET dn_debt_current = (SELECT SUM(acd_sum)
                                     FROM ac_detail, tmp_ac_to_del
                                     WHERE acd_dn = dn_id
                                       AND acd_id = x_acd
                                       AND acd_op = 5
                                       ...
                                       AND acd_start_dt = (SELECT MAX(s.acd_start_dt)
                                                           FROM ac_detail s, tmp_ac_to_del sd
                                                           WHERE s.acd_dn = dn_id
                                                             AND s.acd_id = sd.x_acd
                                                             AND s.acd_op = 5
                                                             AND s.acd_dn = dn_id))
              WHERE EXISTS (SELECT 1 FROM ac_detail, tmp_ac_to_del ad WHERE acd_id = x_acd AND acd_op = 5 AND acd_dn = dn_id)
                AND EXISTS (SELECT 1 FROM tmp_ac_to_del WHERE x_dn = dn_id)
                AND dn_tp = 'D';*/
            --return;
            UPDATE dn_month_usage
               SET dnu_sum =
                         dnu_sum
                       - NVL (
                             (SELECT SUM (acd_sum)
                                FROM tmp_ac_to_del, ac_detail
                               WHERE     x_dn = dnu_dn
                                     AND acd_ac_start_dt <= dnu_stop_dt
                                     AND acd_ac_stop_dt >= dnu_start_dt
                                     AND acd_id = x_acd
                                     AND history_status = 'A'
                                     AND acd_op IN (123, 9)),
                             0)
            ...
            DELETE FROM deduction
                  WHERE     dn_st IN ('E', 'W')
                        ...
                        AND NOT EXISTS
                                (SELECT 1
                                   FROM ac_detail
                                  WHERE acd_dn_src = dn_id);

            --Видаляємо створені в поточному періоді документи повернення надміру стягнених хоштів, якщо видаляються рядки, по яким вони створені
            DELETE FROM ed_log
                  WHERE EXISTS
                            (SELECT 1
                               FROM tmp_ac_to_del, errand
                              WHERE     edl_ed = x_ed
                                    AND x_ed = ed_id
                                    AND ed_st IN ('E', 'P', 'V'));

            DELETE FROM ed_detail
                  WHERE     EXISTS
                                (SELECT 1
                                   FROM tmp_ac_to_del, errand
                                  WHERE     edd_ed = x_ed
        ...
        INSERT INTO tmp_ac_detail (a_pc,
            ...
            SELECT q_pc,
              ...
              FROM (SELECT pd_pc              AS q_pc,
                           ...
                           x_start_dt,
                           x_stop_dt,
                           xpa_bp_start_dt    AS q_ac_start_dt,
                           xpa_bp_stop_dt     AS q_ac_stop_dt,
                           pd_nst             AS q_nst
                      FROM tmp_wac_ids,
                           pc_decision,
                           pd_payment             pdp,
                     ...
                     WHERE     pd_pc = x_id
                           ...
                           AND d_start_dt <= pdp_stop_dt
                           AND d_stop_dt >= pdp_start_dt
        ...
        SELECT COUNT (*), MAX (a_pd)
          INTO l_cnt, l_id
          FROM tmp_ac_detail
         WHERE a_start_dt > a_stop_dt;

        IF l_cnt > 0
        THEN
            raise_application_error (
                -20000,
                   'Помилка обрахування базових нарахувань - розраховано періоди ('
                || l_cnt
                || ' штук)з від`ємною кількістю днів. !max pd='
                || l_id);
        END IF;
        ...
        INSERT INTO uss_esr.tmp_ac_dates1 (tad_pc,
                                           tad_pa,
                                           tad_npt,
                                           tad_dt,
                                           tad_dt1)
            SELECT /*+ full (wac) use_nl(wac pd) use_nl(pd acd) */
             ...
             WHERE     acd_ac = ac_id
                   ...
                   AND acd_op IN
                           (SELECT op_id
                              FROM uss_ndi.v_ndi_op
                             WHERE op_id IN (1, 2, 3) OR op_tp1 IN ('NR'))
                   AND acd_npt = npt.x_id
                   AND acd.history_status = 'A'
                   AND acd_pd = pd_id
                   AND pd_pc = wac.x_id
        ...
        INSERT INTO tmp_ac_dates (td_pc,
                                  ...
                                  td_npt,
                                  td_begin)
            SELECT tad_pc, tad_pa, tad_npt, tad_dt FROM tmp_ac_dates1
            UNION
            SELECT tad_pc, tad_pa, tad_npt, tad_dt1 FROM tmp_ac_dates1;
        ...
        diag_msg ('Знаходимо суми коригувань, доплат та переплат');
        ...
        INSERT INTO uss_esr.tmp_ac_diff (a2_pc,
              ...
              SELECT c_pc,
                ...
                FROM (  SELECT a_pc
                               ...
                               MAX (a_pd)
                                   AS c_pd,
                               SUM (  uss_esr.API$ACCRUAL.xsign (a_op)
                                    * a_sum
                                    * uss_esr.TOOLS.p_o_p (a_start_dt,
                                                           a_stop_dt,
                                                           td_begin,
        ...
        --return;
        diag_msg ('Визначаємо записи, які підуть в проводки без змін');
        ...
        UPDATE tmp_ac_detail
           SET a_need_write_2_base = 'F'
         WHERE a_need_write_2_base IS NULL;

        diag_msg ('Пишему заготовки проводок коригувань');
          ...
          INSERT ALL
            WHEN a2_dn_sum > 0
            THEN             --Пишему суми, які треба перетворити на переплату
                INTO tmp_ac_detail (a_pc,
                                    a_pa,
                                    a_op,
                                    a_npt,
            ...
            SELECT a2_pc,
                   ...
                   --Якщо сума знов нарахованого менше суми виплаченого - є сума переплати
                   CASE
                       WHEN a2_new_calc_sum < a2_old_payed_sum
                       THEN
                           CASE
                               WHEN a2_old_payed_sum - a2_new_calc_sum >
                                    a2_dn_fixed_sum --Якщо новорозрахована сума перепалти більша за вже зафіксовану суму переплати - то різниця є новою переплатою
                               THEN
                                     a2_old_payed_sum
                                   - a2_new_calc_sum
                                   - a2_dn_fixed_sum
        ...
        diag_msg ('Готуємо множини заготовок проводок до запису в таблиці');
        ...
        IF p_calc_mode IN (1, 2)
        THEN
            diag_msg ('Оновлюємо записи accrual');
            ...
            diag_msg ('Додаємо записи ac_detail');
            ...
            INSERT INTO ac_detail (acd_id,
                ...
                SELECT a_id,
                       ...
                       a_pd,
                       CASE
                           WHEN a_dn IS NOT NULL
                           THEN
                               NVL (a_dn_start_dt, a_ac_start_dt)
                           ELSE
                               a_ac_start_dt
                       END,
                       CASE
            ...
            --Формуємо множину переплат, на основі яких створюють відрахування переплат нарахуваннь
            ...
            INSERT INTO tmp_work_ids2 (x_id)
                SELECT a_id
                  FROM tmp_ac_detail
                 WHERE a_need_write_2_base = 'T' AND a_op IN (6, 30);

            diag_msg (
                'Реєструємо заготовки документів переплат по нарахуванням та з-за донарахування держутриманнь');
            API$DEDUCTION.init_deduction_by_accrual (2, g_messages);

            --Формуємо множину документів одноразових доручень для виплати надміру стягнених коштів відрахувань закладам держутримань
            ...
            INSERT INTO tmp_work_ids4 (x_id)
                SELECT a_id
                  FROM tmp_ac_detail
                 WHERE a_need_write_2_base = 'T' AND a_op = 33;

            diag_msg ('33=' || SQL%ROWCOUNT);


            diag_msg (
                'Реєструємо заготовки документів документів одноразових доручень');
            API$ERRAND.init_errand_by_accrual (1);

            diag_msg ('Актуалізуємо дані змінених відрахувань');

            --Оновлюємо поле "поточний борг" у відрахуванні на основі нерознесених залишків по ід-у операцій 5
            DELETE FROM tmp_work_set3
                  WHERE 1 = 1;
                ...
                SELECT dn_id,
                 ...
                 WHERE     EXISTS
                               (SELECT 1
                                 ...
                                 WHERE a_op IN (5, 9) AND a_dn = dn_id)
                       AND EXISTS
                               (SELECT 1
                                  FROM tmp_wac_ids
                                 WHERE dn_pc = x_id)
            ...
            --return;
            --Оновлюємо для тих, по кому знайшлись операцій 5,9 (це ознчає, що нерознесений борг)
            UPDATE deduction
               SET dn_debt_current =
                       (SELECT x_sum1
                          FROM tmp_work_set3
            ...
            --А по тим, по кому є операції взагалі, але немає 5 операції, проставляємо 0, бо це означає, що весь борг погашений
            UPDATE deduction
               SET dn_debt_current = 0
             WHERE     EXISTS
                           (SELECT 1
                              FROM tmp_ac_detail ad
                             WHERE a_dn = dn_id AND a_need_write_2_base = 'T')
                   AND NOT EXISTS
                           (SELECT 1
                              FROM tmp_work_set3
                             WHERE dn_id = x_id1)
            ...
            /*--Оновлюємо поле "поточний борг" у відрахуванні на основі нерознесених залишків по ід-у операції 5
            UPDATE deduction
              SET dn_debt_current = (SELECT SUM(a_sum)
                                     FROM tmp_ac_detail ma
                                     WHERE a_op = 5
                                       AND a_dn = dn_id
                                       AND a_start_dt = (SELECT MAX(s.a_start_dt)
                                                         FROM tmp_ac_detail s
                                                         WHERE s.a_op = 5
                                                           AND s.a_dn = ma.a_dn))
              WHERE EXISTS (SELECT 1 FROM tmp_ac_detail ad WHERE a_op = 5 AND a_dn = dn_id)
                AND EXISTS (SELECT 1 FROM tmp_wac_ids WHERE dn_pc = x_id)
                AND dn_tp = 'D';*/
            ...
            /*--Оновлюємо поле "поточний борг" у відрахуванні на основі рознесених залишків по ід-у операції 123
            UPDATE deduction
              SET dn_debt_current = dn_debt_current - (SELECT SUM(a_sum) FROM tmp_ac_detail WHERE a_op IN (124, CASE WHEN dn_tp = 'D' THEN 124 ELSE 123 END) AND a_dn = dn_id AND a_id > 0)
              WHERE EXISTS (SELECT 1 FROM tmp_ac_detail WHERE a_op IN (124, CASE WHEN dn_tp = 'D' THEN 124 ELSE 123 END) AND a_dn = dn_id AND a_id > 0)
                AND EXISTS (SELECT 1 FROM tmp_wac_ids WHERE dn_pc = x_id)
                AND dn_tp IN ('R', 'HM');*/
            ...
            diag_msg ('Актуалізуємо дані accrual');

            actuilize_payed_sum (1);

            --IF l_hs IS NULL OR l_cnt > 0 THEN
            l_hs := TOOLS.GetHistSession ();
                            ...
                            SUM (
                                CASE
                                    WHEN acd_op NOT IN (5,
                                                        ...
                                                        32)
                                    THEN
                                        DECODE (API$ACCRUAL.xsign (acd_op),
                                                1, acd_sum,
                                                0)
                                    ELSE
                                        0
            ...
            SELECT x_pd,
              ...
              FROM uss_ndi.v_ddn_acdm_tp,
                   (  SELECT acd_pd
                             ...
                             'V1'
                                 AS x_sum_tp,
                             SUM (acd_sum * uss_esr.API$ACCRUAL.xsign (acd_op))
                                 AS x_sum,
                             LISTAGG (acd_id, ',')
                    ...
                    UNION ALL
                      SELECT acd_pd
                       ...
                       WHERE     pd_pc = pc_id
                             ...
                             AND acd_npt NOT IN (166, 167)
                    GROUP BY acd_pd, TRUNC (acd_start_dt, 'MM'))
             WHERE x_sum_tp = dic_value AND (x_sum <> 0 OR x_sum_tp = 'V2');
    END;
    ...
    BEGIN
        ...
        THEN --Включено в відомості АСОПД - дозволити включити в відомості ЄІССС
            SELECT SUM (acd_sum * uss_esr.API$ACCRUAL.xsign (acd_op))
                       AS x_sum,
    ...
    PROCEDURE manipulate_with_acd_sa (p_acd_id   ac_detail.acd_id%TYPE,
                                      p_action   INTEGER) --1=Встановити ознаку в T, 2=Встановити ознаку в F
    IS
        l_acd_id              ac_detail.acd_id%TYPE;
        l_acd_can_use_in_sa   ac_detail.acd_can_use_in_sa%TYPE;
        l_acd_dn              ac_detail.acd_dn%TYPE;
        l_acd_start_dt        ac_detail.acd_start_dt%TYPE;
        l_acd_sum             ac_detail.acd_sum%TYPE;
    BEGIN
        BEGIN
            SELECT acd_id,
                   acd_can_use_in_sa,
        ...
        EXCEPTION
            ...
            THEN
                raise_application_error (
                    '-20000',
                    'Вказано некоректний рядок відрахувань: або це не держутримання, або ви не маєте права робити цю операцію!');
        END;

        IF p_action = 1
        THEN
            IF l_acd_can_use_in_sa = 'F'
            THEN
                UPDATE ac_detail
                   SET acd_can_use_in_sa = 'T'
                 WHERE acd_id = l_acd_id AND acd_can_use_in_sa = 'F';
                ...
                API$DEDUCTION.write_dn_log (
                    ...
                    || l_acd_sum
                    || '#'
                    || TO_CHAR (l_acd_start_dt, 'DD.MM.YYYY')
                    || '#T',
                    NULL,
                    'SYS');
            END IF;
        ELSIF p_action = 2
        THEN
            IF l_acd_can_use_in_sa = 'T' OR l_acd_can_use_in_sa IS NULL
            THEN
                UPDATE ac_detail
                   SET acd_can_use_in_sa = 'F'
                 WHERE     acd_id = l_acd_id
