...
CREATE OR REPLACE PACKAGE USS_ESR.LOAD$ASOPD_34
IS
    -- Author  : JSHPAK
    -- Created : 02.01.2023
    -- Purpose :
...
GRANT EXECUTE ON USS_ESR.LOAD$ASOPD_34 TO USS_EXCH
/


/* Formatted on 8/12/2025 5:49:59 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_ESR.LOAD$ASOPD_34
IS
        ...
        LOOP
            ...
            EXCEPTION
                ...
                THEN
                    l_error_msg :=
                           rec_ls.ls_nls
                        || '; Особовий рахунок містить періоди дії послуги за межами "01.03.2022" та "30.04.2022";';
                WHEN ex_error_34_period_exists
                THEN
                    l_error_msg :=
                           rec_ls.ls_nls
                        || '; Особовий рахунок містить періоди дії послуги які перетинаються з періодом "01.03.2022" та "30.04.2022";';
                WHEN OTHERS
                THEN
                    l_error_msg :=
                           rec_ls.ls_nls
    ...
    BEGIN
                                   ...
                                   ON     pap.pdap_pd = d.pd_id
                                      ...
                                      AND (   rec_np.np_dnprav BETWEEN pap.pdap_start_dt
                                           ...
                                           OR rec_np.np_dkprav BETWEEN pap.pdap_start_dt
                                                                   AND pap.pdap_stop_dt)
                         WHERE d.pd_st IN ('S', 'PS') -- IC #91164 Нам потрібен статус лише S, PS (нараховано, призупинено)
                                                      AND d.pd_pa = p_pa)
                LOOP
        ...
        -- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        -- Первый пост апдейтинг, наведение порядка в периодах pd_payment (на основании метода ПЕР)]
        -- ShY 28112022 удалил - избиточній
        /*for rec in (select
                      m.*,
                      lead(pdp_start_dt) over (partition by m.pdp_npt order by pdp_start_dt) as next_start,
                      case when pdp_stop_dt >= lead(pdp_start_dt) over (order by pdp_start_dt) then 1 else 0 end as is_flag
                    from pd_payment m where m.pdp_pd = l_pd_id)
        loop
          update pd_payment ddd
          set ddd.pdp_stop_dt= rec.next_start -1
          where ddd.pdp_id = rec.pdp_id
            and rec.next_start is not null;
        end loop;*/
            ...
            UPDATE ac_detail ddd
             ...
             WHERE     ddd.acd_pd IN (SELECT pd_id
                   ...
                   AND ddd.acd_op IN (SELECT o.op_id
                                       ...
                                       WHERE o.op_tp1 = 'DN');
        -- старая реализация. пофакту фактическое отчисление никому не интересно (убрано согласно https://redmine.medirent.com.ua/redmine/issues/79816)
        /*
        for rec_udmec  in (select * from v_ls_udmec_data um
                            where um.lfd_lfd = p_lfd_lfd
                              and um.ls_nls = p_ls_nls
                              and um.Ispl_Kud = rec_ispl.ispl_kud
                              and um.Ispl_Num = rec_ispl.ispl_num)
         loop
           insert into dn_detail (dnd_id,dnd_dn,dnd_start_dt,dnd_stop_dt,dnd_tp,dnd_value,history_status,dnd_psc,dnd_value_prefix,dnd_dppa)
           values (Null,l_dn,to_date(lpad(rec_udmec.nudr_mec,2,'0')||'.'||rec_udmec.nudr_god DEFAULT Null ON CONVERSION ERROR, 'mm.yyyy'),
             add_months(to_date(lpad(rec_udmec.nudr_mecf,2,'0')||'.'||rec_udmec.nudr_godf DEFAULT Null ON CONVERSION ERROR, 'mm.yyyy'), 1)-1,
             'AS',rec_udmec.nudr_snac,'A',Null,Null,Null)
           returning dnd_id into l_dnd;
           insert into uss_exch.v_ls2uss(ldr_lfdp,ldr_trg,ldr_code) values(rec_udmec.lfdp_id,l_dnd,'USS_ESR.DN_DETAIL');
         end loop;*/
        END LOOP;
    END;
BEGIN
