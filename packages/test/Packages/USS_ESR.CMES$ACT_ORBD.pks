...
CREATE OR REPLACE PACKAGE USS_ESR.CMES$ACT_ORBD
IS
    -- Author  : BOGDAN
    -- Created : 06.12.2023 11:40:04
    -- Purpose : Формування та перегляд документа «Акт проведення оцінки рівня безпеки дитини»

    Pkg                    VARCHAR2 (50) := 'CMES$ACT_ORBD';


    с_Form_Ndt   CONSTANT NUMBER := 1000;


    TYPE r_Act_Orbd IS RECORD
    (
...
GRANT EXECUTE ON USS_ESR.CMES$ACT_ORBD TO DNET_PROXY
/

GRANT EXECUTE ON USS_ESR.CMES$ACT_ORBD TO II01RC_USS_ESR_PORTAL
/

GRANT EXECUTE ON USS_ESR.CMES$ACT_ORBD TO II01RC_USS_ESR_WEB
/

GRANT EXECUTE ON USS_ESR.CMES$ACT_ORBD TO PORTAL_PROXY
/


/* Formatted on 8/12/2025 5:49:22 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_ESR.CMES$ACT_ORBD
IS
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE Type2xmltable (Pkg, 'r_Act_Orbd', TRUE)
            INTO l_Result
            USING p_Xml;
    ...
    BEGIN
        ...
        THEN
            SELECT COUNT (*)
              ...
              FROM At_Service t
             WHERE     t.Ats_At = l_Act.At_Decision
                   AND t.Ats_Nst = 420
            ...
            THEN
                Raise_Application_Error (
                    -20000,
                    'Створення неможливе. Потрібно щоб надавалась послуга «012.0 Екстрене (кризове) втручання»');
            END IF;

            SELECT COUNT (*)
              INTO l_Already_Exist
              FROM At_Person  p
                   JOIN Act t ON (t.At_Id = p.Atp_At AND t.At_Sc = p.Atp_Sc)
             WHERE     t.At_Id = l_Act.At_Decision
                   AND p.History_Status = 'A'
                   AND FLOOR (MONTHS_BETWEEN (SYSDATE, p.Atp_Birth_Dt) / 12) <
                       18;
            ...
            THEN
                Raise_Application_Error (
                    -20000,
                    'Створення неможливе. Потрібно щоб отримувачем була дитина!');
            END IF;
        END IF;
        ...
        OPEN p_Res FOR
            SELECT a.*,
                   ...
                   Cmes$act.Get_Decline_Reason (a.at_id)
                       AS decline_reason
              FROM Act  a
                   JOIN Uss_Ndi.v_Ddn_At_Orbd_St s ON a.At_St = s.Dic_Value
            ...
            SELECT a.At_Id
             ...
             WHERE     a.At_Tp = 'ORBD'
                   ...
                   AND EXISTS
                           (SELECT 1
                              FROM At_Log l
                             WHERE l.Atl_At = a.At_Id AND l.Atl_St = 'JV');
    ...
    BEGIN
        ...
        Api$act.Get_Form_File (
            ...
            p_At_Prj_St         => 'JN',
            p_Form_Ndt          => с_Form_Ndt,
            p_Form_Build_Proc   => 'Api$Act_Rpt.ACT_DOC_1000_R1',
            p_Doc_Cur           => p_Doc_Cur);
    END;
    ...
    -----------------------------------------------------------
    -- ЗБЕРЕЖЕННЯ ІНФОРМАЦІЇ ЩОДО ПІДПИСАННЯ АКТУ КМом
    -----------------------------------------------------------
    PROCEDURE Set_Signed_Cm (p_At_Id       IN     NUMBER,
                             p_file_code   IN     VARCHAR2,
                             p_Messages       OUT SYS_REFCURSOR)
    ...
    -----------------------------------------------------------
    -- ЗБЕРЕЖЕННЯ ІНФОРМАЦІЇ ЩОДО ПІДПИСАННЯ АКТУ ОТРИМУВАЧЕМ
    -----------------------------------------------------------
    PROCEDURE Set_Signed_Rc (p_At_Id       IN NUMBER,
                             p_file_code   IN VARCHAR2 DEFAULT NULL)
    IS
