...
CREATE OR REPLACE PACKAGE USS_ESR.API$SEARCH
IS
    -- Author  : KELATEV
    -- Created : 29.01.2024 17:51:31
...
CREATE OR REPLACE PACKAGE BODY USS_ESR.API$SEARCH
IS
    g_Sql        CLOB;
    ...
    --допомагає додати свої параметри до запиту
    --(коли параметр буде обовязковий та буде в іншому підзапиті, окремо від #)
    PROCEDURE Add_Bindvar_ (p_Var_Name   IN VARCHAR2,
                            p_Val_Num    IN NUMBER DEFAULT NULL,
                            p_Val_Str    IN VARCHAR2 DEFAULT NULL,
                            p_Val_Dt     IN DATE DEFAULT NULL)
    ...
    BEGIN
        ...
        g_Var_List (g_Var_List.COUNT).Name_ := p_Var_Name;
    ...
    BEGIN
        ...
        Add_Bindvar_ (p_Var_Name   => p_Var_Name,
                      p_Val_Num    => p_Val_Num,
                      p_Val_Str    => p_Val_Str,
                      p_Val_Dt     => p_Val_Dt);
    ...
    BEGIN
        ...
        Add_Sql (
            ...
            p_Var_Name   => NVL (p_Var_Name, p_Field_Name),
            p_Val_Num    => p_Val_Num,
            p_Val_Str    => p_Val_Str,
            p_Val_Dt     => p_Val_Dt);
    END;
