...
CREATE OR REPLACE PACKAGE USS_RNSP.BLD$IMPEXP
IS
    -- Author  : VANO
    -- Created : 13.04.2022 18:55:06
...
CREATE OR REPLACE PACKAGE BODY USS_RNSP.BLD$IMPEXP
IS
    ...
    BEGIN
        /*  FOR xx IN (select 'create or replace synonym &PROXY_USR..'||z.rat_object_name||' FOR &PREFIX..'||z.rat_object_name||';' AS z_synonum_create
                     ...
                     where rat_rsrc = 'RC_USS_RNSP_WEB')
          LOOP
    ...
    BEGIN
        ...
        WL (')');
        WL ('begindata');
    /*  FOR xx IN (SELECT av_id ||'|'||av_tp ||'|'||av_version ||'|'||av_dt ||'|'||av_mimetype ||'|'||av_notes||'|'||av_name ||'|av_content_'||av_id||'.dat|' AS x_row
                 FROM ikis_sys.ikis_app_docs
                 WHERE av_ss_code = 'USS_RNSP'
                   AND av_id IN (SELECT regexp_substr(p_ids_by_comma, '[^,]+', 1, level)
                                 FROM dual
    ...
    */
    ...
    BEGIN
        ...
        WL ('sqlplus uss_rnsp/uss_rnsp@sonya @export_dict_and_config.sql');
    END;
    ...
    BEGIN
        ...
        LOOP
            ...
            WL ('BEGIN');
            WL (
                   '  USS_RNSP.BLD$IMPEXP.make_inserts(''uss_rnsp'', '''
                || tbs (i)
                || ''', '' where 1 = 1'');');
            WL ('END;');
            WL ('/');
    ...
    BEGIN
        ...
        LOOP
            ...
            WL (
                'DELETE FROM USS_RNSP.' || LOWER (tbs (i)) || ' WHERE 1 = 1;');
            WL ('');
        END LOOP;
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user USS_RNSP@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
    ...
    BEGIN
        ...
        l_sqlplus_line :=
            'sqlplus uss_rnsp/uss_rnsp@sonya @make_packages_clob.sql';
    ...
    BEGIN
        ...
        SELECT TRUNC (COUNT (*) / 40) + 1
          INTO l_cnt
          FROM uss_rnsp.rpt_templates;

        l_exportlob_line :=
            'ExportLOBs.exe rpt_templates rt_id rt_text dat BLOB "rt_id in (0';
        l_sqlplus_line :=
            'sqlplus uss_rnsp/uss_rnsp@sonya @make_rpt_templates_ctl.sql';
