...
CREATE OR REPLACE PACKAGE USS_ESR.P$75844886$TMP$ASE
IS
    -- Author  : BOGDAN
...
END P$75844886$TMP$ASE;
/


/* Formatted on 8/12/2025 5:50:04 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_ESR.P$75844886$TMP$ASE
IS
    -- #70663: журнал особових справ
    PROCEDURE GET_JOURNAL (P_PC_NUM               IN     VARCHAR2,
                           P_SC_UNIQUE            IN     VARCHAR2,
    ...
    BEGIN
        ...
        THEN
            l_sql := l_sql || ' AND t.pc_num LIKE ''' || P_PC_NUM || '%''';
        END IF;

        IF P_Org_Id IS NOT NULL AND p_mode IS NULL
        THEN
            l_sql := l_sql || ' AND t.com_org = ' || P_Org_Id;
        END IF;

        IF P_SC_UNIQUE IS NOT NULL
        THEN
            l_sql :=
                l_sql || ' AND sc.sc_unique LIKE ''' || P_SC_UNIQUE || '%''';
        END IF;
        ...
        THEN
            l_sql :=
                   l_sql
                || ' AND t.pc_create_dt >= to_date('''
                || TO_CHAR (P_PC_CREATE_DT_START, 'DD.MM.YYYY')
                || ''', ''DD.MM.YYYY'')';
        END IF;
        ...
        THEN
            l_sql :=
                   l_sql
                || ' AND t.pc_create_dt <= to_date('''
                || TO_CHAR (P_PC_CREATE_DT_STOP, 'DD.MM.YYYY')
                || ''', ''DD.MM.YYYY'')';
        END IF;
        ...
        THEN
            ...
            l_sql :=
                   l_sql
                || ' AND upper(i.sci_ln || '' '' || i.sci_fn || '' '' || i.sci_mn) LIKE '''
                || UPPER (l_pib)
                || '%''';
        END IF;
        ...
        THEN
            l_nis_part :=
                   'AND EXISTS (SELECT * FROM uss_ndi.v_ndi_nis_users zz WHERE zz.nisu_wu = x.com_wu and zz.nisu_nis = '
                || p_nis_id
                || ')';
        END IF;
        ...
        THEN
            l_sql :=
                ...
                || P_Org_Id
                || '
                                          AND ((pd_start_dt <= LAST_DAY(to_date('''
                || TO_CHAR (p_month, 'DD.MM.YYYY')
                || ''', ''DD.MM.YYYY'') ) AND pd_stop_dt >= to_date('''
                || TO_CHAR (p_month, 'DD.MM.YYYY')
                || ''', ''DD.MM.YYYY'') )
                                  ...
                                  and ( exists ( select pd_id as id
                                     ...
                                     or exists ( select at_id as id
                                                  ...
                                                  where at.at_pc = t.pc_id
                                                    and nanc.nanc_ap_tp = ''SS''
                                                    and ( at.at_st in (''SA'', ''O.SA'', ''SP2'', ''SI'', ''SS'', ''SGA'', ''SGM'', ''SGO'', ''SU'', ''SJ'')/*= ''SS''*/)
                                                    )
                                       )
                                  ';
    BEGIN
            ...
            l_sql :=
                ...
                || 'AND exists (SELECT *
                                ...
                                WHERE (sc.sc_id = t.pc_sc)
                                 AND (sd.scd_seria || sd.scd_number = REPLACE('''
                || p_scd_ser_num
                || ''', '' '', '''') ))';
        END IF;
        ...
        OPEN res_cur FOR
            SELECT d.*,
              ...
              FROM v_pc_decision  d
                   JOIN uss_ndi.v_ndi_service_type st
                       ON (st.nst_id = d.pd_nst)
                   JOIN v_appeal ap ON (ap.ap_id = d.pd_ap)
                   LEFT JOIN pd_features t ON (t.pde_pd = d.pd_id)
                   JOIN uss_ndi.v_ndi_pd_feature_type ft
                       ON (ft.nft_id = t.pde_nft AND ft.nft_pt = 288)
             WHERE d.pd_pc = p_pc_id AND ap.ap_tp IN ('SS');
    END;
    ...
    BEGIN
        OPEN res_cur FOR
            /*SELECT t.* --,sc.sc_unique
              FROM Uss_Esr.Me_Dcz_Result_Rows t
              JOIN Uss_Esr.Personalcase Pc ON (Pc.Pc_Id = t.Mdsr_Pc)
             WHERE pc.pc_sc = p_sc_id;*/
            SELECT t.*
              FROM Uss_Esr.Me_Dcz_Result_Rows  t
                   JOIN Uss_Esr.Me_Dcz_Request_Rows r
                       ON r.Mdrr_Id = t.Mdsr_Mdrr
                   JOIN Uss_Person.v_Socialcard Sc
                       ON REPLACE (Sc.Sc_Unique, 'T', '9') =
                          LTRIM (SUBSTR (Mdrr_Id_Fam, -14), '0')
