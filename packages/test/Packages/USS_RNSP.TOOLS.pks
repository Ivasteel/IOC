...
CREATE OR REPLACE PACKAGE USS_RNSP.TOOLS
IS
    -- Author  : VANO
...
GRANT EXECUTE ON USS_RNSP.TOOLS TO DNET_PROXY
/

GRANT EXECUTE ON USS_RNSP.TOOLS TO II01RC_USS_RNSP_WEB
/


/* Formatted on 8/12/2025 5:58:02 PM (QP5 v5.417) */
CREATE OR REPLACE PACKAGE BODY USS_RNSP.TOOLS
IS
    g_curr_org    NUMBER;
    g_curr_root   NUMBER;
    ...
    BEGIN
        RETURN TO_NUMBER (
                   USS_RNSP_CONTEXT.GetContext (USS_RNSP_CONTEXT.gORG));
    END;
    ...
    IS
    BEGIN
        RETURN TO_NUMBER (USS_RNSP_CONTEXT.GetContext ('uid'));
    END;
    ...
    IS
    BEGIN
        RETURN USS_RNSP_CONTEXT.GetContext (USS_RNSP_CONTEXT.gLogin);
    END;
    ...
    BEGIN
        ...
        uss_rnsp_context.SetDnetRnspContext (p_session_id);
    ...
    BEGIN
        ...
        uss_rnsp_context.SetDnetRnspContext (p_session_id, p_ip_address);
        DBMS_SESSION.set_identifier (p_absolute_url);
    END;
    ...
    BEGIN
        INSERT INTO histsession (hs_id, hs_wu, hs_dt)
             VALUES (0, USS_RNSP_CONTEXT.GetContext ('uid'), SYSDATE)
          RETURNING hs_id
               INTO l_hs;
