...
CREATE OR REPLACE FUNCTION IKIS_SYS.DID_MSG$MESSAGE (p_id NUMBER)
    RETURN NUMBER
IS
    l_newid    DECIMAL;
    l_curval   DECIMAL;
...
BEGIN
    ...
    THEN
        ...
        THEN
            BEGIN
                SELECT SQ_DID_MSG$MESSAGE.CURRVAL INTO l_curval FROM DUAL;
            EXCEPTION
                WHEN OTHERS
                THEN
                    SELECT SQ_DID_MSG$MESSAGE.NEXTVAL INTO l_curval FROM DUAL;
            END;
    ...
    SELECT SQ_DID_MSG$MESSAGE.NEXTVAL INTO l_newid FROM DUAL;
