...
CREATE OR REPLACE FUNCTION USS_VISIT.Type2jsontable (
    p_Pkg_Name    VARCHAR2,
...
IS
  ...
  DECLARE
    ...
    BEGIN
      l_Key := Attr2key(p_Attr_Name);
      p_Attr_Val := l_Jo.Get_Clob(l_Key);
    EXCEPTION
    WHEN OTHERS THEN
      Raise_Application_Error(-20000, 'Помилка парсингу поля ' || l_Key || ': ' || SQLERRM);
    END;
    ...
    END;
  BEGIN
    l_jo := Json_Object_t.Parse(:p_Json);
  ...
  END;]';
        ...
        SELECT LISTAGG (
                   CASE
                       ...
                       THEN
                              'IF Attr2key('''
                           || a.Attr_Name
                           || ''') IS NOT NULL THEN EXECUTE IMMEDIATE Type2jsontable('''
                           || a.Attr_Type_Package
                           || ''','''
                           || a.Attr_Type_Name
                           || ''', '''
    ...
    BEGIN
      ...
      l_coll := :pkg_name.:type_name();
      l_ja := Json_Array_t.Parse(:p_Json);
...
----------------------------------------------------------------------
BEGIN
    SELECT TO_NUMBER (SUBSTR (v.Banner, 17, 2))
      INTO l_Db_Version
      FROM V$version v
     WHERE UPPER (v.Banner) LIKE 'ORACLE%';

    --До 18ї версії JSOB_OBJECT_T та JSON_ARRAY_T вміють працювати лиже з JSON у кодуванні UTF8.
    --Тому для старіших версій БД використовуємо реалізацію з приведенням рядків до відповідного кодування
    IF l_Db_Version < 18
    THEN
        RETURN Type2jsontable_12 (p_Pkg_Name, p_Type_Name, p_Date_Fmt);
    END IF;
