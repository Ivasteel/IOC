...
CREATE OR REPLACE FUNCTION IKIS_SYSWEB.DID_W_USERS_HST (p_id NUMBER)
    RETURN NUMBER
IS
    l_newid    DECIMAL;
    l_curval   DECIMAL;
...
BEGIN
    ...
    THEN
        ...
        THEN
            BEGIN
                SELECT SQ_DID_W_USERS_HST.CURRVAL INTO l_curval FROM DUAL;
            EXCEPTION
                WHEN OTHERS
                THEN
                    SELECT SQ_DID_W_USERS_HST.NEXTVAL INTO l_curval FROM DUAL;
            END;
    ...
    SELECT SQ_DID_W_USERS_HST.NEXTVAL INTO l_newid FROM DUAL;
