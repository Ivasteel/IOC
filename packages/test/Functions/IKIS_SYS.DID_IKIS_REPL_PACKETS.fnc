...
CREATE OR REPLACE FUNCTION IKIS_SYS.DID_IKIS_REPL_PACKETS (p_id NUMBER)
    RETURN NUMBER
IS
    l_newid    DECIMAL;
    l_curval   DECIMAL;
...
BEGIN
    ...
    THEN
        ...
        THEN
            BEGIN
                SELECT SQ_DID_IKIS_REPL_PACKETS.CURRVAL
                  INTO l_curval
                  FROM DUAL;
            EXCEPTION
                WHEN OTHERS
                THEN
                    SELECT SQ_DID_IKIS_REPL_PACKETS.NEXTVAL
                      INTO l_curval
                      FROM DUAL;
            END;
    ...
    SELECT SQ_DID_IKIS_REPL_PACKETS.NEXTVAL INTO l_newid FROM DUAL;
