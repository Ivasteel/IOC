...
CREATE OR REPLACE FUNCTION IKIS_SYS.DID_IKIS_EXCEPTION_LOG (p_id NUMBER)
    RETURN NUMBER
IS
    l_newid    DECIMAL;
    l_curval   DECIMAL;
...
BEGIN
    ...
    THEN
        ...
        THEN
            BEGIN
                SELECT SQ_DID_IKIS_EXCEPTION_LOG.CURRVAL
                  INTO l_curval
                  FROM DUAL;
            EXCEPTION
                WHEN OTHERS
                THEN
                    SELECT SQ_DID_IKIS_EXCEPTION_LOG.NEXTVAL
                      INTO l_curval
                      FROM DUAL;
            END;
            ...
            THEN
                RAISE DSERIALS.INVALID_VALUE4DIAPCOLUMN;
            ELSE
                RETURN p_id;
            END IF;
        ELSE
    ...
    SELECT SQ_DID_IKIS_EXCEPTION_LOG.NEXTVAL INTO l_newid FROM DUAL;
