...
CREATE OR REPLACE PACKAGE USS_PERSON.BLD$IMPEXP
IS
    -- Author  : VANO
...
CREATE OR REPLACE PACKAGE BODY USS_PERSON.BLD$IMPEXP
IS
    ...
    */
    ...
    BEGIN
        ...
        DBMS_LOB.append (
            l_result,
               'prompt NVP Medirent Export User Objects for user USS_PERSON@sonya'
            || CHR (13)
            || CHR (10));
        DBMS_LOB.append (
            l_result,
    ...
    BEGIN
        ...
        SELECT TRUNC (COUNT (*) / 10) + 1
         ...
         WHERE     object_type = 'PACKAGE'
               ...
               AND object_name NOT IN ('LOAD$VPO', 'DNET$EXCH_USS2IKIS');

        l_sqlplus_line :=
            'sqlplus uss_person/uss_person@sonya @make_packages_clob.sql';
